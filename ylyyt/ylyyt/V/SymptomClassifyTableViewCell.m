//
//  SymptomClassifyTableViewCell.m
//  ylyyt
//
...
@interface SymptomClassifyTableViewCell ()<UITableViewDelegate,UITableViewDataSource,UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,NSXMLParserDelegate>
{
...
@implementation SymptomClassifyTableViewCell

- (void)awakeFromNib {
    // Initialization code
    UITableView *leftTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, 0.28*SCREEN_WIDTH, 0.55*SCREEN_HEIGHT) style:UITableViewStylePlain];
    ...
    [[AFAppDotNetAPIClient sharedClient] GET:[NSString stringWithFormat:@"api_totalCatalog.xml"] parameters:nil success:^(NSURLSessionDataTask *task, NSXMLParser *responseObject) {
    ...
    UICollectionView *rightCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0.3 *SCREEN_WIDTH, 0, 0.7*SCREEN_WIDTH, SCREEN_HEIGHT-NAVIGATION_HEIGHT_ADD_STATUS_HEIGHT-TABBAR_HEIHGT) collectionViewLayout:flowLayout];
...
- (void)loadCategoryList:(NSString *)categoryId refresh:(BOOL)refresh{
    ...
    [[AFAppDotNetAPIClient sharedClient] GET:[NSString stringWithFormat:@"api_subCatalog.xml"] parameters:nil success:^(NSURLSessionDataTask *task, NSXMLParser *responseObject) {
        responseObject.delegate = (id<NSXMLParserDelegate>)self;
        [responseObject parse];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    [self.delegate pushToSymptomDetail:indexPath.row];
}
