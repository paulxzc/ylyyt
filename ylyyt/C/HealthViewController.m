...
@interface HealthViewController ()<UITableViewDataSource,UITableViewDelegate,NSXMLParserDelegate,loginPopDelegate,showIconDelegate,showPickerDeleagate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,showMessageDelegate>
{
...
//    _loginSuccess = [LoginSuccess loginSuccessWithDic:sender.userInfo];
//    if (_loginSuccess) {
//        [self.tableView reloadData];
//    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0&&self.name == nil) {
        ...
        personCell.delegatePicker = self;
        personCell.delegateMessage = self;
        [personCell refreshCell:_loginSuccess];
        return personCell;
    }else{
...
#pragma mark --showIconDelegate
- (void)showIconAlert:(UIAlertController *)alert{
    [self presentViewController:alert animated:YES completion:nil];
}

#pragma mark --showImagePickerDelegate
...
#pragma mark --showMessageDelegate
...
<<<<<<< HEAD
        ...
        NSData *imageDataFinish = [userDefaults objectForKey:@"userIcon"];

        
        //创建发送通知
        NSNotification *noti = [[NSNotification alloc] initWithName:@"iconRefresh" object:imageDataFinish userInfo:nil];
        [[NSNotificationCenter defaultCenter] postNotification:noti];
